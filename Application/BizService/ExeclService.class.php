...
class ExeclService extends BaseService {
	 ...
	 *		   5,columnAvgWidthArr		- 各列的宽度
	 * 		   6,$columnBgColor		- 首列的默认背景色
	 * @返 回：无（下载到exel文件）
	 * @说 明：当前最多是A-Z列（26列），在数组$columnLetter按Exel列格式添加即可扩展。
	 * @数据格式： $userInfo=array(
	                 1=>array('id','real_name','pmi','email'),
					 2=>array('id','real_name','pmi','email'),
					 3=>array('id','real_name','pmi','email')
	              )
	 * @示 例：PublicModel::downMoreColumnDateToExel($userInfo,'活动参与者信息',array('id','real_name','pmi','email'),array('用户ID号','真实姓名','用户PMI号','用户邮箱''),array(10,15,20,15));
	 */ 
	static public function downMoreColumnDateToExel($data,$fileName,$fields,$columnNameArr,$columnAvgWidthArr,$columnBgColor='#88aa22'){
		set_time_limit(0);
		...
		//设置exel文件属性
		$objPHPExcel->getProperties()->setCreator("ddt")
							 ->setLastModifiedBy("ddt")
							 ->setTitle("ddt")
							 ->setSubject("ddt")
							 ->setDescription("ddt")
							 ->setKeywords("ddt")
    ...
    //垂直居中
    $objPHPExcel->getActiveSheet()->getStyle('A1:'.$columnLetter[$Acount].'1')->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);
    //水平居中
    $objPHPExcel->getActiveSheet()->getStyle('A1:'.$columnLetter[$Acount].'1')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
		...
		for($i=0;$i<$Acount;$i++){
			...
			//设置指定列的宽度
			if(!empty($columnAvgWidthArr)){
				$objPHPExcel->getActiveSheet()->getColumnDimension($columnWord)->setWidth($columnAvgWidthArr[$i]);			//列为设定宽度。
			}else{
