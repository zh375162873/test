...
class DhmManageModel extends Model {
	...
	public function create_dhm($orderinfo,$type){
	     if($orderinfo['is_entity']==1&&$orderinfo['send_type']==1){
		   return true;
		 }
		//初始化数据
    	        ...
    	        if($this->where(array("dhm_code"=>$dhm_code,"store_id"=>$store_id,"shop_id"=>$shop_id))->data(array("status"=>1,"dh_time"=>time()))->save()){
    	           //判断相关订单的核销码还有吗(当订单中的核销码存在退款中也算是订单未完成)
    	            if($this->where(array("order_id"=>$arr['order_id'],"type"=>$arr['type'],"status"=>0,"refund_status"=>array("neq",2)))->find()){
    	                 ...
    	                 if($arr['type']==1){
    	                     D("Home/Orders")->where(array("order_id"=>$arr['order_id']))->data(array("order_status"=>2,"finnshed_time"=>time()))->save();
    	                     $map = array(
    	                          "user_status" => 1,
    	                          "qd_status" => 1,
    	                          "qd_calc_time1" => time(),
	...
	public function list_shop_dhm($shop_id,$store_id,$status=''){
	    ...
	    for($i=0;$i<count($infolist);$i++){
	        ...
	        $infolist[$i]['goods_name']=$arr['goods_name'];
	        $infolist[$i]['goods_price']=$arr['goods_price'];
	        $order = D("Orders")->where(array("order_id"=>$infolist[$i]['order_id']))->field("order_sn,goods_num,order_youhui")->find();
	        $infolist[$i]['order_sn']=$order['order_sn'];
	        //商品价格显示为实际付款价格
	        $infolist[$i]['goods_price']=price_format($arr['goods_price']-($order['order_youhui']/$order['goods_num']));
	...
	public function userChecknum($user_id){
		...
		$data =$this->where('status = 0 AND  buyer_id='.$user_id)->field('goods_id,order_id,dhm_code,add_time')->order('id desc')->select();
		return $data;
