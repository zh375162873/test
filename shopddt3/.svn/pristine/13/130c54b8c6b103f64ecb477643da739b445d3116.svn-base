...
class Page {
    ...
    public function show() {
        ...
        if($nowCoolPage == $this->coolPages){
			...
			if($this->totalPages-$this->nowPage>=$this->rollPage)//判断当前页是否大于5页，大于加5，小于则加1
			{
				$nextRow    =   $this->nowPage+$this->rollPage;
			}else
			{
				$tempage=$this->totalPages-$this->nowPage;
				if($tempage<$this->rollPage)
				{
					$tempage=1;				
				}
				$nextRow    =   $this->nowPage+$tempage;
