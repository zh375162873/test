...
class CommissionOrderModel extends Model {
     ...
     * @param number $type
     */
    public function get_order_list($userid,$pages,$pagesNum=10,$type=1,$daynum){
        if($type==1){
            $num=$pagesNum;
            ...
            $map=array();
            if($daynum){
                $start_time = strtotime(Date('Y-m-d'))-($daynum-1)*86400;
                $end_time = $start_time+$daynum*86400;
                $map['create_time']=array(array("gt",$start_time),array("lt",$end_time),"and");
            }
            $map['referee_id']=$userid;
            
            $arr = $this->where($map)->order("create_time DESC")->limit($limit)->select();
            if(!empty($arr)){
     ...
     * @param number $type 判断统计人员角色1为推广人2渠道
     */
    public function get_shouyi_list($userid,$pages,$pageNum=15,$type=1,$daynum){
        
        if($daynum){
            $page_start = $daynum;
            $start_time = strtotime(Date('Y-m-d'))-($page_start-1)*86400;
            $end_time = $start_time+86400*$daynum;
        }else{
            $num=$pageNum;
            $page_start=$pages*$num;
            $start_time = strtotime(Date('Y-m-d'))-($page_start-1)*86400;
