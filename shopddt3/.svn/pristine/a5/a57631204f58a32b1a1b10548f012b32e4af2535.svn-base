...
class OrdersModel extends AdvModel {
		...
		//初始化订单数据
           ...
           // exit();
			...
			if($result){
				...
				//如果使用优惠码，就在商品详情中保存
				if($money_info['extend_id']&&$money_info['extend_num']){
				    $goodsdate['extend_price']=$goods_arr['extend_price'];
				    $goodsdate['extend_code']=$goods_arr['extend_code'];
					$goodsdate['extend_num']=$goods_arr['extend_num'];
				}
	...
	public function notiy_order($parameter,$type){
		...
		//清理购物车
		$model_cart->clearCart('db',array('buyer_id'=>$info['buyer_id']));
	...
	public function notiy_order_do($parameter,$type){
	   ...
	   $this->_userid=$order_info['buyer_id'];
	 ...
	 * @return Ambigous <\Think\mixed, boolean, unknown>|boolean
	 */
	public function list_order_status0($userid){
	  $data=M()->query("select * from ddt_orders where  buyer_id=$userid and  order_status =0 and order_id=parent_id order by order_id desc");
	  foreach($data as $key=>$val){
	    $order_amount=0;
		$goods_name="";
	    $subdata=M()->query("select * from ddt_orders where  buyer_id=$userid and  order_status =0 and parent_id=".$val['order_id']);
		foreach($subdata as $k=>$vo){
		  $gooddata=$this->ordergoods->where(array("order_id"=>$vo['order_id']))->find();
	      $data[$key]['goods'][$k] = $gooddata;
		  $order_amount=$order_amount+$vo['order_amount'];
		  $goods_name=$goods_name.$gooddata['goods_name']."*".$gooddata['goods_num'].",";
		}
		$data[$key]['order_amount']=$order_amount;
