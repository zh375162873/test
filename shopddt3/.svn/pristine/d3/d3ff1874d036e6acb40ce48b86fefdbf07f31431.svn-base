...
class CommissionController extends BaseController {
	...
	public function ChannelmemberSettlement(){ 
	   ...
	   if($flag==1){
		  $this->success ( '结算成功', U ( 'admin/Commission/ChannelmemberList',array('channelid'=>I('channelid'),"user_id"=>I('user_id')) ) ); 
	   }else{
		 $this->error ("操作失败");
	   }
    }
	...
	public function ChannelmemberPay(){
	  ...
	  if(IS_POST){
	   ...
	   if($flag==1){
		  $this->success ( '支付成功', U ( 'admin/Commission/ChannelmemberList',array('channelid'=>I('channelid'),"user_id"=>I('user_id')) ) ); 
	   }else{
		 $this->error ("操作失败");
	   }
	 }else{ 
