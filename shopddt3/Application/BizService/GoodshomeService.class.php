...
class GoodshomeService extends BaseService { 
	...
	public  function getmgoodssql($data){
		...
		*/
		...
		if($data['gc_id']>0){
		    //查询是否为默认分类
			$class_moren=M('goods_class')->where("(is_all=1 or gc_name='随便看看') and shop_id=".$shop_id." and gc_id=".$data['gc_id'])->find();
			if($class_moren){
			  $sql_order = " a.goods_commonid  desc ";  
			}else{
	...
	public function mgoods_list_bylbs($param=array(),$ispagenation=true){
		...
		// $params['filter']="shop_id:".$shop_id;
		 ...
		 if($param['loadLength']==1){
		  $goodslist_thumb=$goodslist;
		  $array=array();
		  $i=0;
		  foreach($goodslist_thumb as $key=>$val){
		    if($key >6){
		     $array[$i]=$val;
			 $i=$i+1;
			} 
