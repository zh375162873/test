...
class RefundReturnModel extends Model {
        ...
        //判断订单状态是否已经进行了退款了
            ...
            if($this->data($data)->add()){
                if($this->dhm->where(array("id"=>$reason['dhm_id']))->data(array("refund_status"=>1))->save()){
                    //判断订单状态
                    $wxf_dhm = $this->dhm->where(array("order_id"=>$orderid,"status"=>0,"refund_status"=>0))->find();
                    if(empty($wxf_dhm)){
                        $this->orderdb->where(array("order_id"=>$info['order_id']))->data(array("refund_state"=>1))->save();
                    }
                    elseif (!empty($wxf_dhm)){
                        $this->orderdb->where(array("order_id"=>$info['order_id']))->data(array("refund_state"=>4))->save();
                    }
                    return true;
                }else{
                    return false;
    ...
    public function refund_quxiao($order_id,$dhm_id){
        ...
        if($this->dhm->where(array("id"=>$dhm_id,"order_id"=>$order_id))->data(array("refund_status"=>0))->save()){
            /**
                                          状况一：如果存在退款中的核销码就不做任何订单相关的处理
                                          状况二：不存在退款中核销码，如果存在退款完成的核销码就将订单退款状态设为2
                                          状况三：不存在退款中核销码，如果不存在退款完成就将订单退款状态设为0；
             */
            $arr_tkz = $this->dhm->where(array("id"=>$dhm_id,"order_id"=>$order_id,"refund_status"=>1))->find();//退款中
            $arr_tkwc = $this->dhm->where(array("id"=>$dhm_id,"order_id"=>$order_id,"refund_status"=>2))->find();//退款完成
            if(empty($arr_tkz)&&empty($arr_tkwc)){
                $this->orderdb->where(array("order_id"=>$order_id))->data(array("refund_state"=>0))->save();
            }
            elseif(empty($arr_tkz)&&!empty($arr_tkwc)){
                $this->orderdb->where(array("order_id"=>$order_id))->data(array("refund_state"=>2))->save();
            }
